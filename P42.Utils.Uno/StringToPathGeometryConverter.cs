...
public class StringToPathGeometryConverter : IValueConverter
{
    ...
    public PathGeometry Convert(string? path)
        => InternalParse(path) ?? new PathGeometry();
    ...
    /// <param name="path">String with path data definition</param>
    /// <returns>PathGeometry object created from string definition</returns>
    private PathGeometry? InternalParse(string? path)
    {
    ...
    {
        // Allow for a sign 
        var token = _pathString[_curIndex];
        if (signAllowed && More() && token is '-' or '+')
            _curIndex++;

        while (More() && token is >= '0' and <= '9')
        {
            _curIndex++;
        }
    }
    ...
    {
        if (_figureStarted && _figure is not null)
            return _figure;
    ...
    private double ReadNumber(bool allowComma)
    {
        ...
        catch (FormatException except)
        {
            throw new FormatException(
                $"Unexpected character in path '{_pathString}' at position {_curIndex - 1}", except);
        }
    ...
    private bool IsNumber(bool allowComma)
    {
        ...
        if (More())
        {
            ...
            if (_token is '.' or '-' or '+' or 'I' or 'N' || _token is >= '0' and <= '9')
                return true;
    ...
    private bool SkipWhiteSpace(bool allowComma)
    {
        ...
        while (More())
        {
            ...
            switch (ch)
            {
                ...
                default:
                    // Avoid calling IsWhiteSpace for ch in (' ' to 'z']
    ...
    internal static char GetNumericListSeparator(IFormatProvider provider)
    {
        ...
        // If the IFormatProvider does not contain a NumberFormatInfo, then 
    ...
    */
    ...
    {
        if (value is PathGeometry geometry)
