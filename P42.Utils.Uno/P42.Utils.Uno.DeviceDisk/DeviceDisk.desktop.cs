...
public partial class DeviceDisk
{
    public static Task<Dictionary<string, ulong>> NativeGetAppFolderPropertiesAsync(string[] properties)
    {
        return Task.Run(() =>
        {
            ...
            {
                var exitCode = Shell.ExecuteCommand(
                    "wmic",
                    "/locale:ms_409 logicaldisk get name,size,freespace /format:csv",
                    out var output,
                    out var error);
                ...
                if (exitCode != 0)
                {
                    ...
                    QLog.Error(error, "Failed to get app folder properties");
                    return result;
                }
            ...
            {
                var exitCode = Shell.ExecuteCommand(
                    "df","-P .", out var output, out var error);
                if (exitCode != 0)
                {
                    System.Diagnostics.Debug.WriteLine(error);
                    Console.WriteLine(error);
