...
public static class AvailableFonts
{
    ...
    private static string[] GetMacOsSystemFonts()
    {
        ...
        if (Shell.ExecuteCommand("atsutil", "fonts -list ", out var output, out var error) == 0)
        {
            ...
            QLog.Error(error, "Failed to get font system families");
    ...
    private static string[] GetLinuxSystemFonts()
    {
        ...
        if (Shell.ExecuteCommand("fc-list"," : family | sort | uniq ", out var output, out var error) == 0)
            results.AddRange(output.Split(Environment.NewLine).Select(line => line.Trim()));
        else
            QLog.Error(error, "Failed to get system font families");
