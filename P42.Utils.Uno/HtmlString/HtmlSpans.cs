        ...
        void ProcessHTML()
        {
            ...
            for (int i = 0; i < _text.Length; i++)
            {
                if (_text[i] == '<')
                {
                    ...
                    var tagString = _text.Substring(i + (closing ? 2 : 1), j - i - (closing ? 2 : 1));
                ...
                else if (_text[i] == '&')
                {
                    ...
                    var j = i + 1;
                    while (j < _text.Length && _text[j] != ';')
                        escapeCodeBuilder.Append(_text[j++]);
                ...
                {
                    if (inPreSpan || !(char.IsWhiteSpace(_text, i) && i > 0 && char.IsWhiteSpace(_text, i - 1)))
                    {
                        index++;
