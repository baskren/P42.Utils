...
internal static class ColorExtensions
{
    ...
    /// <summary>
    /// Convert to Microsoft.UI.Xaml.Media.Brush
    /// </summary>
    /// <param name="color"></param>
    /// <returns></returns>
    ...
    /// <summary>
    /// Returns color, assuring ability to catch gestures
    /// </summary>
    /// <param name="c"></param>
    /// <returns></returns>
    public static Color AsGestureAble(this Color c)
        => new () { R = c.R, G = c.G, B = c.B, A = Math.Max((byte)0x1,c.A) };

    /// <summary>
    /// Returns brush, assuring ability to catch gestures
    /// </summary>
    /// <param name="b"></param>
    /// <returns></returns>
    public static Brush AsGestureAble(this Brush b)
    {
        if (b is not SolidColorBrush scb)
            return b;

        var color = scb.Color;
        return new SolidColorBrush(color.AsGestureAble());
    }
    #endregion
    ...
    public static Color ColorFromUint(uint argb)
        => new ()
        {
            R = (byte)((argb & 0x00ff0000) >> 0x10), 
            G = (byte)((argb & 0x0000ff00) >> 0x8), 
            B = (byte)(argb & 0x000000ff), 
            A = (byte)((argb & 0xff000000) >> 0x18)
        };
    #endregion
    ...
    public static string ToHexArgbColorString(this Color color)
        => $"{color.A >> 4:x1}{color.R >> 4:x1}{color.G >> 4:x1}{color.B >> 4:x1}";
    ...
    /// <summary>
    /// Returns an 8 character hexadecimal string of a color's AARRGGBB value
    /// </summary>
    /// <returns>The hex rgb color string.</returns>
    /// <param name="color">Color.</param>
    ...
    /// <summary>
    /// Gets color from Application.Current.Resources
    /// </summary>
    ...
    /// <summary>
    /// Gets brush from Application.Current.Resources
    /// </summary>
    /// <param name="key"></param>
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    ...
    /// <summary>
    /// Converts Brush into Color
    /// </summary>
    /// <param name="brush"></param>
    /// <returns>default color if brush is not solid color brush</returns>
