...
public class CacheableNotifiablePropertyObject : SelfBackedNotifiablePropertyObject
{
    ...
    private LocalData.TagItem TagItemKey(string propertyName) => LocalData.TagItem.For(propertyName, Path.Combine(GetType().Name, InstanceIdentifier), GetType().Assembly);
    ...
    protected T? GetCachedValue<T>(string instanceId, T? defaultValue = default, [CallerMemberName] string propertyName = "")
    {
        ...
        var item = TagItemKey(propertyName);
        if (!item.TryRecallValue<T>(out var value))
            value = GetValue(defaultValue, propertyName);
    ...
    protected bool SetCachedValue<T>(string instanceId, T value, [CallerMemberName] string propertyName = "")
    {
        ...
        return TagItemKey(propertyName).TryStoreValue(value);
    }
    ...
    protected void Cache<T>(T value, string instanceId, string propertyName)
    {
        ...
        TagItemKey(propertyName).StoreValue(value);
