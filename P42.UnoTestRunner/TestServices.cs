...
public static partial class TestServices
{
...
#if !WINAPPSDK
        internal static FrameworkElement? GetPopupOverlayElement(Microsoft.UI.Xaml.Controls.Primitives.Popup popup)
        {
            return null;
        }
#endif
    ...
    public static void VERIFY_IS_TRUE(bool value, string message = "")
    {
        Assert.IsTrue(value, message);
    }

    public static void VERIFY_IS_FALSE(bool value, string message = "")
    {
    ...
    internal static void VERIFY_ARE_EQUAL<T>(T actual, T expected, string message = "")
    {
        //Assert.AreEqual(expected: expected, actual: actual);
        actual­.Should().Be(expected, message);
    }

    internal static void VERIFY_ARE_NOT_EQUAL<T>(T actual, T unexpected, string message = "")
    {
        actual­.Should().NotBe(unexpected, message);
    }

    internal static void VERIFY_ARE_VERY_CLOSE(double actual, double expected, double tolerance = 0.1d, string message = "")
    {
    ...
    internal static void VERIFY_DATES_ARE_EQUAL(DateTimeOffset actual, DateTimeOffset expected, string message = "")
    {
        actual.Date.Should().Be(expected.Date, message);
    }

    internal static void VERIFY_DATES_ARE_EQUAL(long actualTicks, long expectedTicks, string message = "")
    {
